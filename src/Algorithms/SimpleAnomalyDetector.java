...
public class SimpleAnomalyDetector implements TimeSeriesAnomalyDetector, AnomalyDetectionAlgorithm{
	...
	public void learnNormal(TimeSeries ts) {
		...
		for(int i = 0; i < ts.values.size(); i++) {
			...
			for(int k = 0; k < firstVal.length; k++)
				firstVal[k] = ts.values.get(i).get(k);

			for(int j = i + 1; j < ts.values.size(); j++) {
				secondVal = new float[ts.values.get(j).size()];
				for(int l = 0; l < secondVal.length; l++)
					secondVal[l] = ts.values.get(j).get(l);
				if(i != j && (Math.abs(currPearson = StatLib.pearson(firstVal, secondVal)) > threshold)) {
			...
			if(maxPearsonIndex != -1) {
				for(int m = 0; m < points.length; m++)
					points[m] = new Point(firstVal[m], ts.values.get(maxPearsonIndex).get(m));
			...
			for (CorrelatedFeatures correlatedFeatures : correlated) {
				...
				if (index1 != -1 && index2 != -1) {
					...
					if (StatLib.dev(p, correlatedFeatures.lin_reg) > (correlatedFeatures.threshold + 0.35)) {
						...
						lst.add(new AnomalyReport(s, i + 1, ts.values.get(ts.valueNames.indexOf(correlatedFeatures.feature1)).get(i + 1), ts.values.get(ts.valueNames.indexOf(correlatedFeatures.feature2)).get(i + 1)));
					}
				}
			}
		}
	...
	public float[] analyzeTimestamps(String ts) {
		...
		while (i < reported.size()) {
			...
			s.append(j).append(",").append(jj).append("\n");
			i++;
		}
	...
	public void drawOnGraph(Canvas canvas, String featureName, int timeStamp) {
		...
		//now draw totalFlightTime l
		ArrayList<AnomalyReport> list = new ArrayList<>(reported);
		list.removeIf(value -> !value.description.contains(featureName) || value.timeStamp > timeStamp);

		//stroke correlation totalFlightTime
